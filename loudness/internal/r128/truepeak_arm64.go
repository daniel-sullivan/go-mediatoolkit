...
// stream with the NEON 2-lane kernel in truepeak_arm64.s. It follows
// the same cursor protocol as oversamplePairGeneric (which it shadows,
// and whose per-lane arithmetic it reproduces bit-for-bit — see the
// assembly's header comment): reads tp.zi, returns the final cursor,
// never writes tp.zi.
...
// oversamplePairAsm is the NEON channel-pair polyphase kernel. z is the
