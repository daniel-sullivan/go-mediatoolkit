...
// Port of libopus/silk/control_SNR.c.
//
// Maps a desired bitrate (bps) to an SNR target (Q7) by looking up a
// precomputed table keyed on the encoder's internal sampling rate.
// The tables hold SNR values divided by 21 so they fit in 8 bits.

// silk_TargetRate_NB_21 — NB (8 kHz) SNR/21 table, 117-10 entries.
var silk_TargetRate_NB_21 = [117 - 10]byte{
	0, 15, 39, 52, 61, 68,
...
// silk_TargetRate_MB_21 — MB (12 kHz) SNR/21 table, 165-10 entries.
var silk_TargetRate_MB_21 = [165 - 10]byte{
	0, 0, 28, 43, 52, 59,
...
// silk_TargetRate_WB_21 — WB (16 kHz) SNR/21 table, 201-10 entries.
var silk_TargetRate_WB_21 = [201 - 10]byte{
	0, 0, 0, 8, 29, 41,
...
// silk_control_SNR — map target bitrate to SNR_dB_Q7 setpoint. C: control_SNR.c:82-113.
func silk_control_SNR(psEncC *silk_encoder_state, TargetRate_bps opus_int32) opus_int {
	var snr_table []byte
	var bound int

	psEncC.TargetRate_bps = TargetRate_bps
	if psEncC.nb_subfr == 2 {
		TargetRate_bps -= 2000 + opus_int32(psEncC.fs_kHz)/16
	}
	switch psEncC.fs_kHz {
	case 8:
		bound = len(silk_TargetRate_NB_21)
		snr_table = silk_TargetRate_NB_21[:]
	case 12:
		bound = len(silk_TargetRate_MB_21)
		snr_table = silk_TargetRate_MB_21[:]
	default:
		bound = len(silk_TargetRate_WB_21)
	...
	if id <= 0 {
		...
		psEncC.SNR_dB_Q7 = opus_int(snr_table[id]) * 21
	}
