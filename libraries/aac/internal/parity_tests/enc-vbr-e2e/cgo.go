...
// Package encvbre2e is the end-to-end AAC-LC VBR encode parity slice. It feeds
// identical int16 PCM frames and an identical AAC-LC VBR config (sample rate,
// channels, AOT=2, a fixed AACENC_BITRATEMODE in 1..5) to BOTH the genuine
// vendored Fraunhofer FDK-AAC encoder (aacEncEncode, TRANSMUX 0 raw access
// units) and the pure-Go internal/nativeaac VBR encoder
// (nativeaac.NewEncoderVBR -> EncodeOneFrame), and asserts the emitted access
// units are BYTE-IDENTICAL. fdk-aac encode is fixed-point (int32 Q-format), so a
// fixed VBR mode reproduces the bitstream bit-for-bit — the comparison is exact
// (no tolerance), as it must be. This is the VBR sibling of encode-e2e and
// exercises the FDKaacEnc_AdaptThresholdsVBR / FDKaacEnc_reduceThresholdsVBR
// threshold-reduction path (the VBR else-branch of FDKaacEnc_AdjustThresholds).
//
...
//
// Scope: AAC-LC (AOT 2), VBR (AACENC_BITRATEMODE 1..5), TRANSMUX 0 raw AUs,
// 1024-sample frames, mono/stereo. EXCLUDED: HE-AAC/SBR, ELD, DRC/metadata,
// user peak bitrate (AACENC_PEAK_BITRATE left default, so no
// FDKaacEnc_AdjustVBRBitrateMode adjustment) — none are configured here.
package encvbre2e
...
// vbr_encode opens a raw-AAC (TRANSMUX 0) AAC-LC VBR encoder for the given
// sample rate / channels / bitrateMode (1..5), encodes framesIn interleaved
// int16 frames one at a time, and returns EVERY produced access unit
// concatenated in au[] with per-AU byte counts in auLens[0:*nAU], plus the ASC
// in asc[0:*ascLen].
//
// This drives the GENUINE vendored aacEncEncode. The bitrate is NOT set: in VBR
// mode the lib derives the nominal bitrate from the mode + channel mode
// (FDKaacEnc_GetVBRBitrate) internally, exactly as nativeaac.NewEncoderVBR does.
// AACENC_AFTERBURNER is left at its default 0 so the byte-stream is the
// deterministic single-pass encode the pure-Go driver port mirrors. Returns 0 on
// success.
static int vbr_encode(int sampleRate, int channels, int bitrateMode,
    ...
    if (aacEncoder_SetParam(enc, AACENC_AOT, 2) != AACENC_OK ||
        ...
        aacEncoder_SetParam(enc, AACENC_BITRATEMODE, (UINT)bitrateMode) != AACENC_OK ||
...
// cEncodeVBR runs the genuine fdk encoder in VBR mode (fixed bitrateMode) over
// framesIn frames and returns every produced access unit (in order) plus the
// ASC. The native encoder under test is replayed against the same PCM
// frame-for-frame.
func cEncodeVBR(sampleRate, channels, bitrateMode, frameLen int, pcm []int16) (aus [][]byte, asc []byte, ok bool) {
	...
	rc := C.vbr_encode(C.int(sampleRate), C.int(channels), C.int(bitrateMode),
