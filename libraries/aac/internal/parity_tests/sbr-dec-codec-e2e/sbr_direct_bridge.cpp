...
// sbr_direct drives the genuine fdk SBR decoder frame-immediate (the SBR payload
// of frame N applied to the core of frame N). NOTE: the genuine fdk FULL HE-AAC
// decoder instead runs the SBR payload one frame BEHIND the core (bsDelay == 1,
// because the default concealment method is ConcealMethodInter — conceal.cpp:270;
// CConcealment_GetDelay returns 1 conceal.cpp:1651; wired to
// SBR_SYSTEM_BITSTREAM_DELAY at aacdecoder_lib.cpp:613/619/1466). That one-frame
// SBR-bitstream delay (plus QMF warmup state) is the entire reported
// HF-near-crossover int16 "divergence" — see the doc atop
// TestSbrDecodeCoreBitExactHF — NOT a native core error.
