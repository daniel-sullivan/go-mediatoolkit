...
// [libraries/aac.Decoder]. It decodes one AAC access unit at a time into a
// scratch frame buffer and copies into the caller's buffer, carrying any
// leftover samples in remainder across Read calls — mirroring
// codec/opus/decoder.go.
type decoder struct {
	pr         PacketReader
	channels   int
	sampleRate int
